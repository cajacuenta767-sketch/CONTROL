...
import { esGestor, veTodo, puedeVender } from '../middleware/auth.js';
...
export function obtenerCliente(id, usuario) {
  ...
  if (usuario && !veTodo(usuario) && c.vendedor_id !== usuario.id) throw prohibido('Este cliente pertenece a otro vendedor');
  return c;
}

export function crearCliente(datos, actor) {
  if (!puedeVender(actor)) throw prohibido('Tu rol no crea clientes');
  const vendedorId = esGestor(actor) && datos.vendedor_id ? datos.vendedor_id : actor.id;
  const r = obtenerDb()
    .prepare('INSERT INTO clientes (nombre, empresa, email, telefono, pais, moneda, notas, vendedor_id, documento_tipo, documento, razon_social, direccion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(datos.nombre, datos.empresa ?? null, datos.email ?? null, datos.telefono ?? null, datos.pais ?? null, datos.moneda ?? 'USD', datos.notas ?? null, vendedorId, datos.documento_tipo ?? null, datos.documento ?? null, datos.razon_social ?? null, datos.direccion ?? null);
...
export function actualizarCliente(id, datos, actor) {
  ...
  const campos = [];
  const valores = [];
  const permitidos = ['nombre', 'empresa', 'email', 'telefono', 'pais', 'moneda', 'notas', 'documento_tipo', 'documento', 'razon_social', 'direccion'];
