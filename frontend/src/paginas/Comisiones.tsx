...
import { api, dinero, fecha, hoy } from '../api';
import { useSesion } from '../sesion';
import { Campo, Estado, Formulario, Indicador, Modal, Tabla, Tarjeta } from '../componentes/ui';
...
export function Comisiones() {
  ...
  const [f, setF] = useState({ vendedor_id: '', desde: hoy().slice(0, 8) + '01', hasta: hoy() });
    ...
    <>
      ...
      <div className="grid-2">
        <Tarjeta titulo="Liquidaciones">
          <Tabla filas={liquidaciones} clave={(l) => l.id} vacio="Sin liquidaciones" onFila={(l) => api.get<any>(`/liquidaciones/${l.id}`).then(setDetalle)} columnas={[
            ...
            { titulo: '', celda: (l) => esSuper && l.estado === 'pendiente' ? <button className="btn ok chico" onClick={async (e) => { e.stopPropagation(); await api.post(`/liquidaciones/${l.id}/pagar`); await cargar(); }}>Marcar pagada</button> : null },
          ]} />
        </Tarjeta>
        <Tarjeta titulo="Detalle de comisiones">
          <Tabla filas={comisiones} clave={(c) => c.id} vacio="Sin comisiones" onFila={(c) => nav(`/ventas/${c.venta_id}`)} columnas={[
            ...
            { titulo: 'Base', celda: (c) => dinero(c.base), alinear: 'derecha' },
            { titulo: '%', celda: (c) => `${c.pct}%`, alinear: 'derecha' },
            { titulo: 'Comisión', celda: (c) => <strong style={{ color: c.monto < 0 ? 'var(--mal)' : undefined }}>{dinero(c.monto)}</strong>, alinear: 'derecha' },
            { titulo: 'Estado', celda: (c) => <Estado valor={c.estado} /> },
          ]} />
        </Tarjeta>
      </div>

      <Modal titulo="Nueva liquidación" abierto={nueva} cerrar={() => setNueva(false)}>
        <Formulario onEnviar={async () => { await api.post('/liquidaciones', { ...f, vendedor_id: Number(f.vendedor_id) }); setNueva(false); await cargar(); }} textoBoton="Crear liquidación" cancelar={() => setNueva(false)}>
