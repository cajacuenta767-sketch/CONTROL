  ...
  return (
    <>
      ...
      <Modal titulo="Nuevo cliente" abierto={abierto} cerrar={() => setAbierto(false)}>
        <Formulario onEnviar={async () => { await api.post('/clientes', { ...nc, email: nc.email || undefined }); setAbierto(false); setNc({ nombre: '', empresa: '', telefono: '', email: '', pais: '', notas: '' }); await cargar(); }} cancelar={() => setAbierto(false)} exito="Cliente creado">
