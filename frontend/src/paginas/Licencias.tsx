...
export function Licencias() {
  ...
  useEffect(() => { api.get<any[]>('/licencias').then(setTodas); api.get<any[]>('/productos').then(setProductos); }, []);
  const set = (k: string, v: string) => { const p = new URLSearchParams(params); if (v) p.set(k, v); else p.delete(k); setParams(p); };
  const conteo = useMemo(() => Object.fromEntries(ESTADOS.map((e) => [e, todas.filter((l) => l.estado === e).length])), [todas]);
  const lista = licencias || [];

  return (
    <>
      <header>
        <div><h1>Licencias</h1><p>{lista.length} licencia(s). Una licencia es una instalación; el equipo se fija en la primera activación.</p></div>
      ...
      <Tarjeta>
        <div className="filtros">
          <div className="chips">
            <button className={`chip ${!filtros.estado ? 'activo' : ''}`} onClick={() => set('estado', '')}>Todas<b>{todas.length}</b></button>
            {ESTADOS.filter((e) => conteo[e] > 0 || filtros.estado === e).map((e) => <button key={e} className={`chip ${filtros.estado === e ? 'activo' : ''}`} onClick={() => set('estado', e)}>{ETIQUETA_ESTADO[e]}<b>{conteo[e]}</b></button>)}
          </div>
        ...
        <Tabla filas={lista} clave={(l) => l.id} onFila={(l) => nav(`/licencias/${l.id}`)}
          vacio={licencias === null ? 'Cargando…' : todas.length === 0 ? <Vacio icono="🔑" titulo="Todavía no hay licencias" texto="Se emiten al registrar una venta." accion={<Link to="/ventas/nueva" className="btn chico">+ Nueva venta</Link>} /> : 'Ninguna licencia coincide'}
          columnas={[
