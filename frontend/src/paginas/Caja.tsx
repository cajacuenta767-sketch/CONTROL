...
import { AccionConMotivo, Aviso, Campo, Estado, Formulario, Indicador, Tabla, Tarjeta } from '../componentes/ui';
...
export function Caja() {
  ...
  return (
    <>
      <header><div><h1>Caja</h1><p>Cada vendedor cierra su caja al final del día. {esSuper ? 'Tú apruebas u observas cada cierre.' : 'El superadministrador la aprueba.'}</p></div></header>

      <Tarjeta titulo="Caja del día" acciones={
        ...
        {dia && (
          <>
            ...
            {dia.cierre ? (
              ...
              <Formulario onEnviar={cerrar} textoBoton={`Cerrar caja del ${fecha(fechaDia)}`}>
      ...
      <Tarjeta titulo="Historial de cierres">
        <Tabla filas={cierres} clave={(c) => c.id} vacio="Sin cierres" columnas={[
          { titulo: 'Fecha', celda: (c) => fecha(c.fecha) },
          ...(esGestor ? [{ titulo: 'Vendedor', celda: (c: any) => c.vendedor_nombre }] : []),
          { titulo: 'Cobrado', celda: (c) => dinero(c.total_cobrado), alinear: 'derecha' },
          { titulo: 'A entregar', celda: (c) => dinero(c.a_entregar), alinear: 'derecha' },
          { titulo: 'Comisión', celda: (c) => dinero(c.comision_dia), alinear: 'derecha' },
          { titulo: 'Cobros', celda: (c) => c.cantidad_pagos, alinear: 'derecha' },
          { titulo: 'Estado', celda: (c) => <><Estado valor={c.estado} />{c.observacion && <span className="suave pequeno"> · {c.observacion}</span>}</> },
          { titulo: '', celda: (c) => esSuper && c.estado !== 'aprobado' ? (
            <div className="fila">
              <button className="btn ok chico" onClick={() => revisar(c.id, 'aprobado')}>Aprobar</button>
              <AccionConMotivo titulo="Observar cierre" texto="Observar" className="btn secundario chico" etiquetaMotivo="Qué no cuadra" onConfirmar={(m) => revisar(c.id, 'observado', m)} />
            </div>
          ) : null },
        ]} />
      </Tarjeta>
      <p className="suave pequeno">Las comisiones no se descuentan de la caja: se pagan por <Link to="/comisiones">liquidación</Link>.</p>
    </>
  );
}
